  ...
  return (
    <>
      ...
      <div className="flex-1 max-w-[1600px] w-full mx-auto px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* Deal stages - vertical timeline */}
        <Card className="p-6" id="stages">
          <p className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground mb-5">Typical deal stages</p>
          <div className="relative space-y-4">
            <div className="absolute left-3 top-2 bottom-2 w-px bg-border" aria-hidden="true" />
            {DEAL_STAGES.map((s, idx) => (
              <div key={s.stage} className="relative flex gap-4 pl-0">
                <div className="h-6 w-6 rounded-full bg-primary text-primary-foreground grid place-items-center text-xs font-semibold tabular-nums shrink-0 relative z-10 shadow-sm">
                  {idx + 1}
                </div>
                <div className="flex-1 min-w-0 pb-1">
                  <p className="font-medium text-sm mb-0.5">{s.stage.replace(/^\d+\. /, '')}</p>
