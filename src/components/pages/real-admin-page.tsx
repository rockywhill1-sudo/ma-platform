      ...
      <div className="flex-1 max-w-[1600px] w-full mx-auto px-6 lg:px-8 py-6 space-y-6">
              ...
              <form action={updateAppSettings} className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-1">Smaller text below the platform name in the sidebar (e.g., "by NeuralEdge").</p>
                    <input type="text" name="platform_tagline" defaultValue={settings.platform_tagline} placeholder="by NeuralEdge" className="w-full px-3 py-2 rounded-md border bg-background text-sm" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-1">Changes the primary color across the entire app.</p>
                    <select name="color_theme" defaultValue={settings.color_theme} className="w-full px-3 py-2 rounded-md border bg-background text-sm">
                ...
                <div>
                  <p className="text-xs text-muted-foreground mb-2">Theme previews:</p>
                  <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                    {Object.values(THEMES).map((t) => (
                      <div key={t.key} className={'rounded-lg border p-2 ' + (settings.color_theme === t.key ? 'border-primary ring-2 ring-primary/20' : '')}>
                        <div className="h-8 rounded-md mb-1.5" style={{ background: t.swatch }} />
                        <p className="text-[10px] font-medium truncate">{t.name}</p>
                        <p className="text-[9px] text-muted-foreground truncate leading-tight">{t.description}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <p className="text-xs text-muted-foreground italic">Tip: After saving, do a hard refresh (Ctrl+Shift+R) to see the theme update everywhere immediately.</p>
        ...
        {company && (
          <Card className="p-6 border-destructive/30">
            <p className="text-[11px] font-mono uppercase tracking-widest text-destructive mb-1">Danger zone</p>
            <p className="text-sm text-muted-foreground mb-4">
              Permanently delete this workspace and all of its data. This cannot be undone.
            </p>
            <a href={"/companies/" + companyId + "/delete"} className="inline-flex items-center gap-2 px-4 py-2 rounded-md border border-red-300 text-red-600 bg-red-50 hover:bg-red-100 text-sm font-medium">Delete this workspace</a>
          </Card>
        )}
      </div>
    </>
